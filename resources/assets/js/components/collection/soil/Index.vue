  ...
  <content-component>
      <el-col :lg="24">
          <el-breadcrumb id="breadcrumb" seprator="/">
              ...
              <el-breadcrumb-item>土壤管理</el-breadcrumb-item>
          </el-breadcrumb>
      </el-col>
      <el-col :lg="3">
      ...
      <el-col :lg="4" class="pull-right">
          <router-link to="/collection/soil/create">
          ...
          <el-table :data="list.data" border style="width: 100%" v-loading="loading" element-loading-text="拼命加载数据中"
              ...
              <el-table-column fixed="right" label="操作" width="200">
                  ...
                  <el-button type="text" size="small">
                          <router-link :to="'/collection/soil/' + scope.row.id + '/edit'">编辑</router-link>
                      </el-button>
                      <el-button type="text" size="small"
...
//            }
    ...
    methods : {
        load() {
            //console.log('load');
            this.loading = true;
            axios.get('/api/collection/soil', this.options)
                .then(response => {
        ...
        handleDeleteRow(index, id, data) {
            this.$confirm('此操作将永久删除该土壤，是否继续？', '提示', {
        ...
        handleBatchDelete() {
            var ids = new Array();
            for (var i in this.batch.soil) {
                ids.push(this.batch.soil[i].id);
            }
            this.batch.params.ids = ids;
            axios.post('/api/collection/soil/batch-delete', this.batch.params)
                .then(response => {
                    this.load();
                }).catch(error => {
        ...
        handleImportSave() {
            ...
            axios.post('/api/collection/soil/import', this.dialog.import.model)
                .then((response) => {
                    this.$message('导入成功');
        ...
        handleImageClick(id) {
            this.dialog.image.prefix = 'api/collection/soil/' + id;
        ...
        //拍摄清单
        handleCaremaList() {
          window.open('/api/soil/camera-list', '_blank');
        }
    }
  }
</script>
